...
interface FormStateType {
  isFormOpen: boolean;
  editItem: Advisors | null;
  items: Advisors[];
}

export default function TeamsAdvisors() {
  ...
  async function loadAdvisors() {
    try {
      ...
      console.log(res)
      setFormState((s) => ({ ...s, items: res.data.advisors ?? [] }));
    } catch (e) {
  ...
  return (
    <>
      <div className="blade-top-padding">
        <SectionHeading
          heading="Section - Teams (Advisors)"
...
type AdvisorsFormValues = z.infer<typeof advisorsSchema>;

function AdvisorsForm({
  initalData,
  onClose,
}: {
  initalData: Advisors | null;
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<AdvisorsFormValues>({
    resolver: zodResolver(advisorsSchema),
    defaultValues,
  });

  const submitHandler: SubmitHandler<AdvisorsFormValues> = async (data) => {
    try {
      setIsLoading(true);
      ...
      if (initalData?.id) {
        await axios.put(
          `${process.env.NEXT_PUBLIC_HOST_URL}/teams/advisors/${initalData.id}`,
          formData,
          {

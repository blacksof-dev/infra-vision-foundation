...
interface FormStateType {
  isFormOpen: boolean;
  editItem: Fellows | null;
  items: Fellows[];
}

export default function TeamsFellows() {
  ...
  async function loadTrustees() {
    try {
      ...
      setFormState((s) => ({ ...s, items: res.data.fellow ?? [] }));
    } catch (e) {
          ...
          >
            <div className="flex gap-4">
              ...
              {item.popupImage && (
                <div className="w-full h-[15rem] rounded-md overflow-hidden border border-gray/20">
                  <img
                    src={`${process.env.NEXT_PUBLIC_HOST_URL}${item.popupImage}`}
...
type FellowsFormValues = z.infer<typeof Fellowschema>;

function FellowsForm({
  initalData,
  onClose,
}: {
  initalData: Fellows | null;
  ...
  const defaultValues = useMemo(() => {
    ...
    return {
      ...
      popupImage: initalData.popupImage || "",
    } as FellowsFormValues;
  }, [initalData]);
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<FellowsFormValues>({
    resolver: zodResolver(Fellowschema),
    defaultValues,
  });

  const submitHandler: SubmitHandler<FellowsFormValues> = async (data) => {
    try {
      setIsLoading(true);
      ...
      // formData.append("order", String(data.order));
       formData.append("subtitle", String(data.subtitle));
      ...
      if (data.popupImage instanceof FileList && data.popupImage.length > 0) {
  formData.append("popupImg", data.popupImage[0]); // <-- FIXED
}
      if (initalData?.id) {
        await axios.put(
          `${process.env.NEXT_PUBLIC_HOST_URL}/teams/fellow/${initalData.id}`,
          formData,
          {

...
interface FormStateType {
  isFormOpen: boolean;
  intialValue: OAEDefaultValueType;
}
  ...
  return (
    <>
      <section className="blade-top-margin pb-10">
        <SectionHeading
          heading="Outreach & Engagements Banner"
        ...
        <div className="mt-10">
          <OutreachCard data={formState.intialValue} />
        </div>
      </section>
...
function OutreachForm({
  ...
  initalData: OAEDefaultValueType;
  onClose: () => void;
  onSuccess: () => void;
}) {
  const { data: session } = useSession();
  ...
  const submitHandler: SubmitHandler<OutreachFormValues> = async (data) => {
    try {
      setIsLoading(true);
      ...
      if (desktopValue instanceof FileList && desktopValue.length > 0) {
        const result = await uploadImage(
          desktopValue[0],
          session,
          `outreach-banner-desktop-${Date.now()}`,
        );
        if (!result.success) {
      ...
      if (mobileValue instanceof FileList && mobileValue.length > 0) {
        const result = await uploadImage(
          mobileValue[0],
          session,
          `outreach-banner-mobile-${Date.now()}`,
        );
        if (!result.success) {
...
function OutreachCard({ data }: { data: OAEDefaultValueType }) {
  const {
    backgroundImageDesktop,
    backgroundImageMobile,
    heading,

"use client";
import { useState, useMemo, useRef, useEffect } from "react";
import { UnderlineWithHover } from "@/_components/atoms/buttons";
import { NewsCard } from "@/_components/molecules/newsCard";
import { useApiHook } from "@/lib/useApi";
import { getUrl } from "@/lib/getUrl";
import { TabApiRaw } from "./02_researchPapers";

// Types
type FilterType = "All" | "Publication year" | "Sectors";

export interface Blog {
  id: string;
  title: string;
  subtitle: string;
  slug: string;
  author: string;
  readingTime: number;
  publishedDate: string;
  coverImage: string;
  active: boolean;
  sectors: {
    id: string;
    name: string;
    slug: string;
    active: boolean;
  }[];
}

interface BlogsApiResponse {
  blogs: Blog[];
  pagination: {
    totalCount: number;
    page: number;
    limit: number;
    totalPages: number;
  };
}

const FILTER_TYPES: FilterType[] = ["All", "Publication year", "Sectors"];
const YEARS = ["2025", "2024", "2023"] as const;
const PAGE_LIMIT = 3;

function formatDate(dateStr: string): string {
  if (!dateStr) return "";
  const date = new Date(dateStr);
  return date.toLocaleDateString("en-US", {
    year: "numeric",
    month: "long",
    day: "numeric",
  });
}

function getYearFromDate(dateStr: string): string {
  if (!dateStr) return "";
  return new Date(dateStr).getFullYear().toString();
}

export default function Blogs() {
  const tabRefs = useRef<Array<HTMLButtonElement | null>>([]);
  const containerRef = useRef<HTMLDivElement>(null);

  // Top-level tab: All | Publication year | Sectors
  const [selectedTab, setSelectedTab] = useState<FilterType>("All");
  // Sub-filter value (year or sector name)
  const [selectedFilter, setSelectedFilter] = useState<string>("All");

  const [records, setRecords] = useState<{
    [key: string]: { page: number; cards: Blog[]; totalCount: number };
  }>({});

  // Fetch sectors for the Sectors sub-filter
  const { data: tabsData } = useApiHook<TabApiRaw[]>({
    url: "/knowledge/sectors",
    cacheKey: "knowledgeBlogSectorTab",
  });

  const sectorList = useMemo(
    () => ["All", ...(tabsData?.map((tab) => tab.name) ?? [])],
    [tabsData],
  );

  // Build a unique cache key based on current tab + sub-filter
  const currentKey = `${selectedTab}-${selectedFilter}`;

  const currentData = records[currentKey] ?? {
    page: 1,
    cards: [],
    totalCount: 0,
  };

  const getApiUrl = (): string => {
    // Sector filter: fetch by sector id
    if (selectedTab === "Sectors" && selectedFilter !== "All") {
      const sector = tabsData?.find((t) => t.name === selectedFilter);
      if (sector) {
        return `/knowledge/blogs/by-sector/${sector.id}?page=${currentData.page}&limit=${PAGE_LIMIT}`;
      }
    }
    // All other cases: fetch all blogs (filtered client-side for year)
    return `/knowledge/blogs?page=${currentData.page}&limit=${PAGE_LIMIT}`;
  };

  const { data: cardData } = useApiHook<BlogsApiResponse>({
    url: getApiUrl(),
    cacheKey: `knowledgeBlogData-${currentKey}-page-${currentData.page}`,
  });

  useEffect(() => {
    if (cardData?.blogs && cardData.pagination?.page === currentData.page) {
      setRecords((prev) => {
        const prevState = prev[currentKey] ?? {
          page: 1,
          cards: [],
          totalCount: 0,
        };

        const newCards = cardData.blogs;

        if (currentData.page === 1) {
          return {
            ...prev,
            [currentKey]: {
              page: 1,
              cards: newCards,
              totalCount: cardData.pagination.totalCount,
            },
          };
        }

        // For subsequent pages, filter out duplicates
        const existingIds = new Set(prevState.cards.map((c) => c.id));
        const uniqueNewCards = newCards.filter((c) => !existingIds.has(c.id));

        if (uniqueNewCards.length === 0) {
          return prev;
        }

        return {
          ...prev,
          [currentKey]: {
            page: currentData.page,
            cards: [...prevState.cards, ...uniqueNewCards],
            totalCount: cardData.pagination.totalCount,
          },
        };
      });
    }
  }, [cardData, currentKey, currentData.page]);

  // Filter cards client-side for Publication year
  const displayedCards = useMemo(() => {
    if (selectedTab === "Publication year") {
      return currentData.cards.filter(
        (card) => getYearFromDate(card.publishedDate) === selectedFilter,
      );
    }
    return currentData.cards;
  }, [currentData.cards, selectedTab, selectedFilter]);

  const scrollToCenter = (index: number) => {
    const tab = tabRefs.current[index];
    const container = containerRef.current;
    if (tab && container) {
      const offset =
        tab.offsetLeft - container.offsetWidth / 2 + tab.offsetWidth / 2;
      container.scrollTo({ left: offset, behavior: "smooth" });
    }
  };

  const handleTabClick = (tab: FilterType) => {
    setSelectedTab(tab);
    // Reset sub-filter to sensible default per tab
    const defaultFilter =
      tab === "Publication year" ? YEARS[0] : tab === "Sectors" ? "All" : "All";
    setSelectedFilter(defaultFilter);
    // Reset records for the new key if not present
    setRecords((prev) => ({
      ...prev,
      [`${tab}-${defaultFilter}`]: { page: 1, cards: [], totalCount: 0 },
    }));
  };

  const handleFilterClick = (filterName: string, index: number) => {
    setSelectedFilter(filterName);
    scrollToCenter(index);
    setRecords((prev) => ({
      ...prev,
      [`${selectedTab}-${filterName}`]: { page: 1, cards: [], totalCount: 0 },
    }));
  };

  const handleSeeMore = () => {
    setRecords((prev) => ({
      ...prev,
      [currentKey]: {
        ...currentData,
        page: currentData.page + 1,
      },
    }));
  };

  // Allow seeing more if we haven't loaded all items yet
  const canSeeMore =
    currentData.totalCount > 0 &&
    currentData.cards.length < currentData.totalCount;

  const renderFilterButtons = (filters: readonly string[]) => (
    <div ref={containerRef} className="pt-5 overflow-scroll no-scrollbar">
      <div className="flex gap-3">
        {filters.map((filter, index) => (
          <button
            key={filter}
            ref={(el: HTMLButtonElement | null): void => {
              tabRefs.current[index] = el;
            }}
            className={`text-base text-nowrap cursor-pointer rounded-[50px] px-3 py-1 sm:px-6 sm:py-3
              ${
                selectedFilter === filter
                  ? "border border-pink text-white bg-pink font-medium"
                  : "border border-lightgray/30"
              }`}
            onClick={() => handleFilterClick(filter, index)}
          >
            {filter}
          </button>
        ))}
      </div>
    </div>
  );

  return (
    <section id="blogs">
      <div className="w-container blade-top-padding-sm blade-bottom-padding-lg">
        {/* Header Section */}
        <div className="flex flex-row items-center gap-2 md:gap-3">
          <span className="w-[7px] h-[7px] md:w-[15px] md:h-[15px] rounded-full bg-pink"></span>
          <h5 className="font-medium text-pink">Blogs</h5>
        </div>

        <div className="py-3 max-w-4xl">
          <h1 className="text-black font-light">
            Championing infrastructure issues
            <br />
            <span className="text-black font-medium">
              {" "}
              and recommending solutions
            </span>
          </h1>
        </div>

        {/* Filter Section */}
        <div className="pt-5">
          {/* Top-level tabs: All | Publication year | Sectors */}
          <div className="flex flex-col sm:flex-row gap-6 border-b border-darkgray/20">
            <div className="sm:border-r sm:border-darkgray/20">
              <h5 className="text-darkgray/80 sm:py-5 pr-5 text-nowrap">
                Filter by
              </h5>
            </div>

            <div className="flex flex-row gap-5">
              {FILTER_TYPES.map((tab) => (
                <button
                  key={tab}
                  className={`mt-auto text-base cursor-pointer rounded-[50px] px-4 py-2 mb-3 sm:px-6 sm:py-3 sm:mb-4
                    ${
                      selectedTab === tab
                        ? "border border-pink text-pink font-medium"
                        : "border border-lightgray/30"
                    }`}
                  onClick={() => handleTabClick(tab)}
                >
                  {tab}
                </button>
              ))}
            </div>
          </div>

          {/* Sub-filter buttons */}
          {selectedTab === "Publication year" && renderFilterButtons(YEARS)}
          {selectedTab === "Sectors" && renderFilterButtons(sectorList)}

          {/* Blog Cards */}
          <div className={`${selectedTab !== "All" ? "pt-8" : "pt-8"}`}>
            {displayedCards.length === 0 && cardData && (
              <div className="flex justify-center">No results</div>
            )}
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5 sm:gap-10 xl:gap-16 xlg:gap-24">
              {displayedCards.map((card) => (
                <div key={card.id}>
                  <NewsCard
                    date={formatDate(card.publishedDate)}
                    title={card.title}
                    image={getUrl(card.coverImage)}
                    link={`/blogs/${card.slug}`}
                    category={card.sectors[0]?.name ?? ""}
                    description={""}
                    classes="line-clamp-3"
                    ctaType="read more"
                  />
                </div>
              ))}
            </div>

            {/* See More */}
            {canSeeMore && (
              <div className="flex justify-center sm:mt-4 mb-4">
                <UnderlineWithHover
                  size="xxlsize"
                  color="pink"
                  bgColor="pink"
                  text="See more"
                  role="button"
                  borderColor="white"
                  handlefun={handleSeeMore}
                />
              </div>
            )}
          </div>
        </div>
      </div>
    </section>
  );
}

...
console.log(primaryEvent.details.images?.[0]?.image)

  return (
    <>
      <div className="relative  md:p-4  bg-white  rounded-lg   flex flex-col sm:flex-row items-cente sm:gap-4 md:gap-7 xl:gap-10 min-h-[22rem] lg:min-h-[25rem] overflow-hidden">
        <div className="w-full h-[15rem] sm:h-[20rem] lg:h-[22rem] sm:w-[45%] lg:w-[38%]  sm:p-5  relative">
          <Image
            src={getImageUrl(primaryEvent.details.images?.[0]?.image)}
            alt={primaryEvent.meetingType}
            fill
            quality={100}
            className="rounded-lg w-full h-full object-cover object-top m-3"
          />
        ...
        <div className=" px-3 py-6 xl:py-0 w-full sm:w-1/2 flex flex-col justify-center ">
          <h2 className="text-pink font-semibold lg:text-[42px]">
            {primaryEvent.meetingType}
          </h2>

          <div className="max-w-sm sm:pt-2 xlg:pt-6">
            <h2 className="font-medium pt-2 lg:pt-4 xl:pt-6 line-clamp-3">
              {primaryEvent.desc}
            </h2>

            <div className=" mt-2 sm:mt-4">
              <h4 className=" text-pink">{apiDateConversion(primaryEvent.date)}</h4>
